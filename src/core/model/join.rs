...
pub struct NeighborReq {
    ...
    /// receiver-owned, and sent untransformed. see [`Direction`] for the rule.
...
pub struct NeighborRes {
    ...
    pub level: LookupTableLevel,
    /// receiver-owned, and echoed back from the request untransformed, because it reports on the
    /// responder's own table rather than instructing a write. see [`Direction`] for the rule.
    ///
    /// no logic reads this field today, only a tracing field, since the round trip correlates on
    /// `nonce` alone, so a responder's value for it is never checked. the first consumer that keys
    /// off it must compare it against the direction that was requested.
    pub direction: Direction,
    /// the responder's neighbor entry at the queried level and direction, if populated.
    pub neighbor: Option<Identity>,
}

/// asks the receiver to adopt the candidate as its neighbor on the given direction and
...
pub struct LinkReq {
    ...
    /// neighbor at this direction and level; the receiver either installs it in its own
    /// table or forwards the request when an existing neighbor sits between them.
    pub candidate: Identity,
    /// receiver-owned. neither the sender nor a forwarding hop transforms it. see [`Direction`]
    /// for the rule.
    pub dir: Direction,
...
pub struct LinkRes {
    /// correlation nonce, set once by the originator and threaded unchanged through every hop.
    pub nonce: Nonce,
    /// receiver-owned, and inverted by the sender before this reply goes out, since it names the
    /// recipient's own slot rather than the one the sender wrote. see [`Direction`] for the rule.
    pub dir: Direction,
    ...
    /// from the candidate's point of view, its new neighbor at this direction and level;
    /// `None` means no candidate exists on this direction at this level.
    pub linked: Option<Identity>,
}

/// searches for the candidate's "buddy": the nearest node on the given direction whose
...
/// [`LinkRes`] means the chain ran out — no buddy exists on this direction at this level.
#[derive(Debug, Copy, Clone)]
pub struct BuddyReq {
    /// correlation nonce, set once by the originator and threaded unchanged through every hop.
    pub nonce: Nonce,
...
/// a periodic repair probe asking the receiver "does your `dir` slot at `level`
